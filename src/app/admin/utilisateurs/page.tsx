    ...
    <>
        ...
        <div className="flex items-center justify-between px-8 py-4">
          <div>
            <div className="flex items-center gap-1.5 text-xs text-brand-400 mb-1">
              <span>Admin</span><span>›</span><span className="text-brand-700 font-medium">Équipe</span>
            </div>
            <h1 className="font-display text-[22px] font-bold text-brand-950">Utilisateurs</h1>
          </div>
        </div>
      </header>
      ...
      <div className="px-8 py-6 space-y-5">
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: "1rem" }}>
          {KPI_CARDS.map((k) => (
            <div key={k.label} className="card p-5 flex items-center gap-4">
              <div className={"h-10 w-10 rounded-xl grid place-items-center shrink-0 " + k.color}>
                <k.icon className="h-5 w-5" />
              </div>
              <div>
                <p className="text-xs text-brand-500">{k.label}</p>
                <p className="font-display text-2xl font-bold text-brand-950 tabular-nums leading-tight">{k.value}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="rounded-xl border border-brand-100 bg-white overflow-hidden">
          ...
          <div className="px-6 py-3 border-t border-brand-50 text-xs text-brand-400">
